...
## Statuses
Statuses express the current stage of resolution a ticket is in, and each specific status belongs to one of four canonical categories. Every organisation ships with a **system status** for each category: **New** for a freshly created ticket that no one has yet taken on, **In Progress** for active work, **Waiting** for tickets blocked on external input, and **Done** for closed tickets. These four system rows are seeded automatically, translated into every supported locale, cannot be renamed or deleted, and are the ones AI and internal automation dispatch to when they only know the target category.
On top of the system defaults, each organisation can add its own **custom statuses** inside any category — for example "For Analysis" or "In Testing" under *In Progress*, or "Waiting for client" under *Waiting*. Custom statuses appear in the picker after the system row of their category and can be renamed, reordered, or removed at any time through *Settings → Issue states*. Because AI and automations key off the category (not the label), adding your own refinements never breaks the automatic fallback.
> ℹ️ Transitions between statuses are performed using a picker in the ticket detail. The picker always lists the four system rows first (marked with a small lock icon) followed by any custom statuses defined for the organisation; a footer link opens the settings page for managing the custom set.
## Assignment
Each ticket has two participants with distinct roles. **Reporter** is the person who created the ticket — typically a customer for external inquiries or a colleague for internal tasks. The reporter remains permanently on the ticket, even if they have moved to another department in the meantime, as they are the source of the issue and the recipient of the final response. **Assignee**, on the other hand, is the person currently responsible for progress and can be changed at any time — handing over a ticket between colleagues is a common part of the workflow, and previous assignments remain visible in the activity history.
If no one is assigned, the ticket awaits triage. Such tickets are visually distinguishable in the filter, so the team knows what needs to be distributed.
...
The context menu on each row and in the detail view offers three key actions for routine work:
...
The AI workflow starts automatically immediately after each new ticket is created and can also be run manually from the context menu — typically if the ticket has changed in the meantime and you want the AI to re-evaluate its content.
