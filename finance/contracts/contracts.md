...
Contracts are a formal record of agreements that an organization enters into with other parties — with employees, clients, suppliers, and business partners. The `/contract` module unifies this area: it provides a single central place where a contract can be drafted, reviewed, finalized into an unalterable PDF, its validity tracked, and after its term ends, transferred to the archive. This eliminates fragmented document storage across emails and shared drives, and each agreement has a clear history from draft to signature. The module handles both standard documents (employment contracts, NDAs, service agreements, supplier contracts) and one-off customized legal texts. Integration with the contact directory ensures that the identity of the contracting parties is always current and consistent with other system modules.
